...
useEffect(()=>{
  const fetchWeather = async()=>{
    ...
    await getFormattedWeatherData({...query,units}).then(data =>{
      toast.success(`Successfully Fetched weahter for ${data.name},${data.country}`);
      setWeather(data);
  ...
  const formatBackground = ()=>{
    ...
    const threshold = units === "metric" ? 15: 45
  ...
  return (
    ...
    <ToastContainer autoClose={5000} theme="coloured" position="top-right" newOntop={true} />
